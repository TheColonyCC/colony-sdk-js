...
integration("pagination iterators", () => {
  ...
  it("iterPosts yields unique posts up to maxResults", async (ctx) => {
    try {
  ...
  it("iterPosts with colony filter only yields matching posts", async (ctx) => {
    try {
      const posts: Post[] = [];
      for await (const post of client.iterPosts({
        colony: "general",
  ...
  it("iterPosts stops on a partial page (no extra request)", async (ctx) => {
    try {
